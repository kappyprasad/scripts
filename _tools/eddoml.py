...
def prettyPrintXML(d, indent='', output=sys.stdout, verbose=False):
    nestPrintXML(d,indent,output,verbose)
    return

def nestPrintXML(d, indent='', output=sys.stdout, verbose=False):
    t='%s'%type(d)
    if verbose:
        sys.stderr.write(t)
    if t == 'org.python.core.PyDictionary' or t == '<type \'dict\'>':
        ...
        for i in range(len(keys)):
            ...
            nestPrintXML(d[key], indent='%s    ' % indent, output=output, verbose=verbose)
            output.write('</%s>\n'%element)
        output.write('%s' %indent)
    elif t == 'org.python.core.PyList' or t == '<type \'list\'>':
        bracketPrintXML(d, '    %s'%indent, output, verbose=verbose)
...
def bracketPrintXML(d, indent, output=sys.stdout, verbose=False):
    for i in range(len(d)):
        output.write('%s<item>' % (indent))
        nestPrintXML(d[i], indent='%s' % indent, output=output,verbose=verbose)
        output.write('%s</item>' % (indent))
    return
